...
| **CISA Known Exploited Vulnerabilities (KEV)** | Actively-exploited vulnerability catalog feeding the ontology's `cyber-vuln` objects, alert rail, and operational brief (`src/data/kevCatalog.js`) | U.S. public government work; CISA requests no attribution but courtesy credit is given | "Source: CISA Known Exploited Vulnerabilities Catalog" (courtesy) |
| **NASA EONET** (Earth Observatory Natural Event Tracker) | Curated open natural events (storms, volcanoes, wildfires, ice) as `natural-event` ontology objects (`src/data/eonetEvents.js`) | U.S. public domain (NASA); [EONET API docs](https://eonet.gsfc.nasa.gov/docs/v3) | "NASA EONET — Earth Observatory Natural Event Tracker" |
| **NOAA Space Weather Prediction Center (SWPC)** | Planetary K-index + R/S/G storm scales as the `space-weather` ontology object (`src/data/spaceWeather.js`) | U.S. public domain (NOAA) | "NOAA Space Weather Prediction Center" |
| **Copernicus Data Space Ecosystem (STAC)** | Sentinel-1/Sentinel-2 acquisition METADATA (footprints, times, thumbnails) as `imagery` ontology objects (`src/data/sentinelCatalog.js`) — scene pixels are never fetched | [Copernicus full, free and open data policy](https://dataspace.copernicus.eu/); attribution required | "Copernicus Data Space Ecosystem" (attribution required) |
| **ADS-B navigation accuracy (via adsb.lol / OpenSky feeds)** | Statistical GPS-interference detection — degraded NACp share per grid cell (`src/data/gpsJamming.js`). Derived analytic over the same public ADS-B records already credited above | ODbL 1.0 (underlying adsb.lol records) | "adsb.lol contributors (ODbL) — derived interference analysis by GEV" |
